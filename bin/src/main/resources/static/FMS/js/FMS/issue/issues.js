...
function saveIssuesDetails() {
	...
	var strUrl = "http://localhost:8085/FleetManagement/issuesController/insertAndUpdateIssues";
	$.ajax({
		type : 'POST',
		url : strUrl,
...
function editIssues(serialid, vehiclename, issueType, issuedate, issueSummary,
		issueDesc, issueReportBy, issueAssignBy, odometer) {
	...
	issueVerfiedBy();
	console.log("editIssues function calling....>" + issueReportBy);
	$('#uvehicleid').val();
	$('#ureported_date').val(issuedate);
	$('#issue_type_id').val();
	$('#summary').val(issueSummary);
	$('#udescription').val(issueDesc);
	$('#uodometer').val(odometer);
	$('#ulalbel_id').val();
	$('#ureport_id').val();
	$("#uvehicleid option:contains(" + vehiclename + ")").attr('selected',
			'selected').trigger("chosen:updated");
	$("#uissue_type_id option:contains(" + issueType + ")").attr('selected',
			'selected').trigger("chosen:updated");
	$("#ureport_id option:contains(" + issueReportBy + ")").attr('selected',
			'selected').trigger("chosen:updated");
	$("#uassign_to option:contains(" + issueAssignBy + ")").attr('selected',
			'selected').trigger("chosen:updated");
...
function searchIssuesList() {
	$("#deleteButtonID").hide();
	console.log("get Issues List function calling...")
	...
	if (vehicleid == null && issueType == null) {
	...
	var strUrl = "http://192.168.1.191:8085/FleetManagement/issuesController/searchIssuesDetails";
	$.ajax({
		type : 'POST',
		data : JSON.stringify(objson),
		url : strUrl,
