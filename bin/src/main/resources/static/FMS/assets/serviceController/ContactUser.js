					...
					if (200 !== responsecode) {
						...
						window
								.setTimeout(
										function() {
											window.location
													.replace("http://192.168.1.106:2000/FMS/Pages/Contact&Users/contactlist.html");
										}, 2000);
					           }
...
function getContactUsers() {
	$("#deleteButtonID").hide();
	var strUrl = 'http://192.168.1.106:2000/FleetManagement/contactUsersController/getContactList';
	console.log("" + strUrl);
	$.ajax({
		type : 'GET',
		url : strUrl,
...
function searchContactUsers(empType) {
	...
	if(nameid==null&&emailId==null&&phoneNo==null){
	 ...
	 var objJson = {
	    ...
	    var strUrl = 'http://192.168.1.106:2000/FleetManagement/contactUsersController/searchContactDetails';
	    console.log("" + strUrl);
	    $.ajax({
	        type: "POST",
	        url: strUrl,
...
function getUserRoles() {
	$('#role_id').empty();
	// $('#employeeTypeUpdate').empty();
	var strUrl = 'http://localhost:2000/FleetManagement/contactUsersController/getUserRoles';
	$.ajax({
		type : 'GET',
		url : strUrl,
		dataType : 'json',
