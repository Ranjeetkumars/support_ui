...
//getAssignedVehicle Based On District. 
 function getVehicleBasedOnDistrictForSearch(districtid) {
		...
		//var strUrl = 'http://192.168.1.191:5050/MobileMedicalUnit/ServicePointRegistrationControler/getAssignedVehicleBasedOnDistrict';
			...
			success : function(data) {
				...
				if (200 !== responsecode) {
					...
					$.each(jsonArray, function(i, resData) {
						var vehicle = "<option value=" + resData.vehicleName
...
//getAssignedVehicle Based On District. 
 function getVehicleBasedOnDistrict(districtid) {
		...
		//var strUrl = 'http://192.168.1.191:5050/MobileMedicalUnit/ServicePointRegistrationControler/getVehicleBasedOnDistrict';
...
//Service Point Name Function Calling.
 function getServicePointName(districtid) {
		...
		//var strUrl = 'http://192.168.1.191:5050/MobileMedicalUnit/ServicePointRegistrationControler/getServicePoint';
		$.ajax({
			type : 'POST',
    		url : strUrl,
    		data : JSON.stringify(objson),
			...
			success : function(data) {
				...
				if (200 !== responsecode) {
					...
					$.each(jsonArray, function(i, resData) {
						var vehicle = "<option value=" + resData.servicepointId
...
//Service Point Name Function Calling for update.
 ...
 function insertServicePointSchdule(){
	 ...
	 var dateofvisit=$('#dateofvisitid').val();
	 ...
	 if(servicepointid=="0"){
		 showNotificationError("Please Select Service Point", "servicepointid", "error");
		 return; 
	 }
	 if(dateofvisit==null||dateofvisit==""){
		 showNotificationError("Please Enter Date Of Visit", "dateofvisitid", "error");
		 return; 
	 }
	 ...
	 //var strUrl = 'http://192.168.1.191:5050/MobileMedicalUnit/vehicleToDistrictMappingController/insertMMUSchedule';
	$.ajax({
		type : 'POST',
		url : strUrl,
		data : JSON.stringify(objson),
        ...
        showNotificationError("Successfully Inserted", "registration_id", "success");
...
//update service point schdule
 function updateServicePointSchdule(){	 
	 ...
	 if(dateofvisit==null||dateofvisit==""){
		 showNotificationError("Please Enter Date Of Visit", "udateofvisitid", "error");
		 return; 
	 }
	 if(start_time==null||start_time==""){
	...
	$.ajax({
		...
		success : function(data) {
        console.log("Successfully Inserted");
        showNotificationError("Successfully Updated", "update_id", "success");
        window.setTimeout(function () {
            location.reload();
        }, 2000);			
		},
 ...
 function validateDataOfSearchSchduleDetails(){
	var district_id=$('#sdistrictid').val();
	 if(district_id=="-1" || district_id=='-1' ||district_id=='undefined' || district_id==undefined){
 ...
 function searchSchduleData(){
	  $('#get_schdule').empty();
	    ...
	    try {
	    	...
	    	//var strUrl = 'http://192.168.1.191:5050/MobileMedicalUnit/vehicleToDistrictMappingController/getServicepointSchduling';
	        $.ajax({
	        	type : 'POST',
	    		url : strUrl,
	    		data : JSON.stringify(objson),
 ...
 function getSchduleDetails() {
	    ...
	    try {
	       // var strUrl=Service.getSchduleDetails;
	    	var strUrl = 'http://192.168.1.191:5050/MobileMedicalUnit/vehicleToDistrictMappingController/getServicepointSchdulingForSearch';
 ...
 //reset search schdule details
	...
	// Table Data Appending Here
	        ...
	        for (var i = 0; i < strData.length; i++) {
	           ...
	           var tablcol6 = document.createElement("td");
	            ...
	            $(tablcol8).append('<u><a href="#" data-toggle="modal"data-placement="bottom" data-target="#update" title="View"><i	class="fa fa-edit"></i></a></u>');
	            $(tablcol8).attr('onclick', 'getSingleRowData("' + strData[i].servicepointid + '","' + strData[i].districtId + '","' + strData[i].dateof_visit + '","' + strData[i].starttime + '","' + strData[i].endtime + '","' + strData[i].vehicleId + '","' + strData[i].distname + '","' + strData[i].serialid + '")');
	...
	function getSingleRowData(servicepointname,district,dateofvisit,startTime,endTime,vehicleid,distname,serialid){
