...
$('#end_date').change(function() {
	...
	if (start_date === '' || start_date === "") {
		showNotificationError('Please select from date', 'end_date', 'error');
		return false;
	}
...
$('#update_ToDate').change(function() {
	...
	var FromDate = moment(update_FromDate, 'YYYY-MM-DD');
	var ToDate = moment(update_ToDate, 'YYYY-MM-DD');
...
function get_MyLeave_History_Details() {
	...
	if (startDate === '') {
		showNotificationError('Please select from date', 'search_start_date',
				'error');
		return false;
	} else {
	...
	if (endDate === '') {
		showNotificationError('Please select to date', 'search_end_date',
				'error');
		return false;
	} else {
...
function Get_ManagerName() {
	...
	$.ajax({
		...
		success : function(data) {
			var responsecode = data.responseCode;
			if (200 !== responsecode) {
				// alert("aaa No Data Found");
			} else {
				var jsonArray = data.objLeaveApplyControllerDTO;
				console.log(jsonArray);
				$.each(jsonArray, function(i, resData) {
					console.log(resData);
					$('#manager_Id').val(resData.user_id);
...
function Get_Eraned_Leave_Details() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				$.each(jsonArray, function(i, resData) {
					...
					if (eraned_count[0] === '0' || eraned_count[0] === "0"
							|| eraned_count[0] === undefined
							|| eraned_count[0] === "NA"
							|| eraned_count[0] === ""
							|| eraned_count[0] === null) {
						$('#eraned_Leave').html('0');
		...
		error : function(err) {
			console.error("Error in Get_Eraned_Leave_Details"
					+ JSON.stringify(err));
		}
	});
}
...
function Get_Casual_Leave_Details() {
	...
	$.ajax({
		...
		error : function(err) {
			console.error("Error in Get_Casual_Leave_Details"
					+ JSON.stringify(err));
		}
	});
}
...
function Get_Compassionate_Leave_Details() {
	...
	$.ajax({
		...
		error : function(err) {
			console.error("Error in Get_Compassionate_Leave_Details"
					+ JSON.stringify(err));
		}
	});
}
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				var jsonArray = data.objLeaveApplyControllerDTO;
				$.each(jsonArray, function(i, resData) {
					var leave_type = "<option value=" + resData.leavetypeid
							+ ">" + resData.leave_description + "</option>";
		...
		error : function(err) {
			console.error("Error in get_Leave_Type_DropDown"
					+ JSON.stringify(err));
		}
	});
}
...
function get_Leave_Status_DropDown() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				$.each(jsonArray, function(index, resData) {
					var leave_status = "<option value=" + resData.leave_id
							+ ">" + resData.leave_statustype + "</option>";
		...
		error : function(err) {
			console.error("Error in get_Leave_Status_DropDown"
					+ JSON.stringify(err));
		}
	});
}
...
function insert_Leave_Details() {
	var us_mangerId = $('#manager_Id').val();
	...
	if (leave_Type_Id === '' || leave_Type_Id === "0"
		...
		showNotificationError('Please select leave type', 'leave_error',
				'error');
		return false;
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				showNotificationError('Leave Request Applied',
						'save_Leave_Details', 'success');
				setTimeout(function() {
					window.location.reload();
				}, 3000);
			}
		...
		error : function(err) {
			console
					.error("Error in insert_Leave_Details"
							+ JSON.stringify(err));
		}
	});
}
...
function update_Leave_Details() {
	...
	var update_FromDate = $('#update_FromDate').val();
	if (update_FromDate === '') {
		showNotificationError('Please select from date', 'update_FromDate',
				'error');
		return false;
	} else {
	...
	var update_ToDate = $('#update_ToDate').val();
	if (update_ToDate === '') {
		showNotificationError('Please select to date', 'update_FromDate',
				'error');
		return false;
	} else {
	...
	if (update_LeaveType === '' || update_LeaveType === "0"
			|| update_LeaveType === undefined) {
		showNotificationError('Please select leave type', 'update_error',
				'error');
		return false;
	}
	...
	console.log('::::: UPDATE JSON OBJECT :::::'
			+ JSON.stringify(json_Update_Leave_Details));
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				showNotificationError('Update Succesfully', 'update_Leave_ID',
						'success');
				setTimeout(function() {
					window.location.reload();
				}, 3000);
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				var jsonArray = data.objLeaveApplyControllerDTO;
				$.each(jsonArray, function(index, resData) {
					Update_Leave__UserCancel_Details(leave_Apply_Id,
							resData.leave_createddtm, resData.from_date,
							resData.to_date, resData.leave_noofleavedays,
							resData.leavetype, resData.remarks);
					update_Leave_SheduledAllocated_Details(leave_Apply_Id,
							resData.from_date, resData.to_date);
		...
		error : function(err) {
			console
					.error("Error in Leave_Status_Details"
							+ JSON.stringify(err));
		}
	});
}

function update_Leave_SheduledAllocated_Details(leave_checked_applyid,
		from_date, to_date) {
	...
	$.ajax({
		...
		error : function(err) {
			console.error("Error in update_Leave_SheduledAllocated_Details"
					+ JSON.stringify(err));
		}
	});
}

function Update_Leave__UserCancel_Details(leave_Apply_Id, leave_createddtm,
		from_date, to_date, leave_noofleavedays, leavetype, remarks) {

	var cancel_Remraks = $('#cancel_Remraks').val();
	if (cancel_Remraks === "" || cancel_Remraks === undefined) {
		showNotificationError('Please enter remarks', 'Cancel_Leave_ID',
				'error');
		return false;
	}
	...
	if (reamrks_Length === 200) {
		showNotificationError('Maximum 200 characters only allowing',
				'Cancel_Leave_ID', 'error');
		return false;
	}
	var json_Leave__UserCancel_Details = {
		"remarks" : cancel_Remraks,
		"leave_id" : leave_Apply_Id,
		"leave_createdbyid" : user_Id, // temporraory Purpose
		"leave_createdbymoduleid" : moduleid, // temporraory Purpose
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				Update_Leave__Approval_Details(leave_Apply_Id, remarks,
						leave_createddtm, from_date, to_date,
						leave_noofleavedays, leavetype, remarks);
				showNotificationError('Leave Cancelled Succesfully',
						'Cancel_Leave_ID', 'success');
				setTimeout(function() {
					window.location.reload();
				}, 3000);
			}
		...
		error : function(err) {
			console.error("Error in json_Leave__UserCancel_Details"
					+ JSON.stringify(err));
		}
	});
}

function Inserting_Email_Details(leave_Apply_Id, remarks_Id, apply_date,
		from_date, to_date, noof_leavedays, leave_type, reasons) {
	...
	var message = 'Dear .'
			...
			+ ',<br><br>'
			+ manager_Name
			+ ' Has Approved Leave Request Canceled : '
			+ leave_Apply_Id
			+ '<br><br>Please find the below Details<br>Applied On: \n\
           '
			+ apply_date
			+ ' <br><b>From Date:  '
			+ from_date
			...
			+ '<br>Remarks : '
			+ remarks_Id
			+ '<br><br>NOTE: As per leave policy,any applied/approved leave cancellation is possible within 7 days from the end date of leave. Beyond 7 days, \n\
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				showNotificationError('Mail Sended Succsefully', 'remarks_Id',
						'success');
				Get_Phone_Number();
...
function Inserting_Sms_Details(leave_Apply_Id) {
	...
	var subject = "Leave Request Canceled on Leave Request ID: "
			+ leave_Apply_Id + " ";
	...
	console.log(' :::: SMS JSON OBJECT ::::'
			+ JSON.stringify(json_Inserting_Sms_Details));
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				showNotificationError('Sms Sended Succsefully',
						'Cancel_Leave_ID', 'success');
...
function Update_Leave__Approval_Details(leave_Apply_Id, remarks_Id,
		creadted_dtm, from_date, to_date, no_ofleavedays, leave_type, remraks) {
	...
	console.log('::::: JSON OBJECT :::::'
			+ JSON.stringify(json_Leave__Approval_Details));
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responsecode) {
				...
				Inserting_Email_Details(leave_Apply_Id, remarks_Id,
						creadted_dtm, from_date, to_date, no_ofleavedays,
						leave_type, remraks);
			}
		},
		error : function(err) {
			console.error("Error in json_Leave__Approval_Details"
					+ JSON.stringify(err));
		}
	});
}
	...
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			if (leave_statustype === "Approved"
					|| leave_statustype === "Rejected") {
				$(tablcol11)
						.html(
								'<div class="row"> <div class="col-sm-6"> <a href="#" class="disabled" data-toggle="modal"  data-toggle="tooltip"  data-placement="bottom" data-target="#update_Modal" title="Edit"><span class="glyphicon glyphicon-pencil" style="color:blue"></span></a> </div><div class="col-sm-6"><a href="#" class="disabled" data-toggle="modal" data-toggle="tooltip"  data-placement="bottom" title="Cancel" data-target="#update_Modal1"> <span class="glyphicon glyphicon-remove" style="color:red"></span> </a></div></div>');
				$(tbleRow).append(tablcol11);
			} else {
				$(tablcol11)
						.html(
								'<div class="row"> <div class="col-sm-6"> <a href="#" data-toggle="modal"  data-toggle="tooltip"  data-placement="bottom" data-target="#update_Modal" title="Edit"><span class="glyphicon glyphicon-pencil" style="color:blue"></span></a> </div><div class="col-sm-6"><a href="#" data-toggle="modal" data-toggle="tooltip"  data-placement="bottom" title="Cancel" data-target="#update_Modal1"> <span class="glyphicon glyphicon-remove" style="color:red"></span> </a></div></div>');
				$(tablcol11).attr(
						'onclick',
						'get_RowData("' + leave_apply_id + '","' + from_date
								+ '","' + to_date + '","' + noof_leavedays
								+ '","' + remarks + '","' + leavetypeid + '")');
				$(tbleRow).append(tablcol11);
			}
			// Appending Body Here
...
function get_Leave_Cancel_Details_DOM(strData) {
	...
	try {
		...
		var objTHead8 = document.createElement("th");
		$(objTHead8).html("Approved Manager");
		$(objTHead8).addClass("text-center");
		$(objTr).append(objTHead8);
		...
		for (var i = 0; i < strData.length; i++) {
			...
			if (leave_statustype === "Approved"
				...
				$(tablcol11)
						.html(
								'<div class="row"> <div class="col-sm-6"> <a   class="disabled" href="#" data-toggle="modal"  data-toggle="tooltip"  data-placement="bottom" data-target="#update_Modal" title="Edit"><span class="glyphicon glyphicon-pencil" style="color:blue"></span></a> </div><div class="col-sm-6"><a href="#" class="disabled" data-toggle="modal" data-toggle="tooltip"  data-placement="bottom" title="Cancel" data-target="#update_Modal1"> <span class="glyphicon glyphicon-remove" style="color:red"></span> </a></div></div>');
				$(tbleRow).append(tablcol11);
			} else {
				$(tablcol11)
						.html(
...
function get_RowData(leave_apply_id, from_date, to_date, noof_leavedays,
		remarks, leavetype) {
