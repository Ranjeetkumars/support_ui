...
function getStatesBasedOnCountryId(countryId, zone_stateId) {
	console.log("zone_stateId ========>" + zone_stateId);
	try {
		var id = '#' + zone_stateId;
		//$('#up_zone_stateId').empty();
		$(id).empty();
...
function insertZoneDetails() {
	...
	if (ZoneName === null || ZoneName === '' || ZoneName === "") {
		...
		showNotificationError("Select Country", "zoneNameid", "error");
		return;
	}
		...
		showNotificationError("Select District", "zone_stateId", "error");
		return;
	}
	else if (mandal === "0" || mandal === '0') {
		showNotificationError("Select Mandal", "zone_mandalId", "error");
		return;
	}
	else if (cityId === "0" || cityId === '0') {
		showNotificationError("Select City", "zone_mandalId", "error");
		return;
	}
	else if (locality === "0" || locality === '0') {
		showNotificationError("Select Locality", "zone_localityId", "error");
		return;
	}
	else if (longitude === null || longitude === '' || longitude === "") {
		showNotificationError("Please Enter Longitude", "zone_localityId", "error");
		return;
	}
	else if (lattitude === null || lattitude === "" || lattitude === '') {
		showNotificationError("Please Enter Lattitude", "zone_lattitudeId", "error");
		return;
	}
	else if (description === null || description === "" || description === 'undefine') {
		showNotificationError("Please Enter Description", "zone_lattitudeId", "error");
		return;
	}
	...
	$.ajax({
		...
		success: function(data) {
			if (data !== null || data !== 0) {
				showNotificationError("Inserted Successfully", "insert_ZoneId", "success");
				getZonedetails();
				setTimeout(function() {
...
function get_ZoneDataForUpdate(z_locationId, locationName, countryId, stateId, districtId, mandalId, cityId, localityId, longitude, lattitude, desc) {
	...
	getStatesBasedOnCountryId(stateId, 'up_zone_stateId');
...
function deleteZone(locationId) {
	...
	if (confirm("Are you sure, do you want delete this zone?")) {
		var locationId = locationId;
		var createdbyId = user_Id;
		var createdbymoduleId = module_Id;
		var createdbyroleId = role_Id;
...
function zone_Characters(event) {	
	$("#zoneNameid").keydown(function(e) {
...
function desc_Characters(event) {	
	$("#zone_descriptionId").keydown(function(e) {
		if (e.shiftKey || e.ctrlKey || e.altKey) {
			e.preventDefault();
		} else {
			var key = e.keyCode;
