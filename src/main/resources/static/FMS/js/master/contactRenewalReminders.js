...
$(document).ready(function() {
	try {
		getAllContactRenewalRemindersRef();
 ...
 * 2020-04-20
 */
function addContactRenewalRemindersRegistration() {

	var contactRenewalType = $('#contactRenewalTypeId').val();
	...
	var objJson = { "condition":1,
		       "renewal_typeid":1, 
			      "renewal_type":contactRenewalType,
	...
	var strUrl = Service.addContactRenewalRemindersRegistration;
	console.log("addVehicleTypeRegistration  details Url is:"
			+ strUrl);
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not Inserted ",
						"contactRenewalRemindersRegistrationId", "error");

			} else {

				showNotificationError("Inserted Successfully",
						"contactRenewalRemindersRegistrationId", "success");
				getAllContactRenewalRemindersRef() ;
		...
		error : function() {
			...
			console.log('In Error of  addContactRenewalRemindersRegistration');
		}
	});
}

function getAllContactRenewalRemindersRef() {

	$("#contactRenewalRemindersId").empty();
	var strUrl = Service.getContactRenewalTypeDetails;
			...
			.ajax({
				...
				success : function(data) {
					...
					if (200 !== responseCode || data.status === "NO_DATA_FOUND") {
						allVehicleTypeData_DOM_NO_DATA();
						...
						if (jsonArray.length > 0) {
							allVehicleTypeRefData_DOM(jsonArray);
							loadDataTable();
						}
					}
				},
...
function allVehicleTypeData_DOM_NO_DATA() {
	...
	$(objTHead3).html('Contact Renewal Type');
	...
	$("#contactRenewalRemindersId").append(objDivTag);
}
function allVehicleTypeRefData_DOM(strData) {
	...
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			$(tablcol3).append(strData[i].renewal_type);;
		...
		console.log("contactRenewalRemindersId" + err);
	}
}
var renewal_typeId;
function get_RowData(renewal_typeid, renewal_type) {
	...
	$('#up_contactRenewalTypeId').val(renewal_type);

}

function  updateContactRenewalTypeRegistration(){

	var contactRenewalType = $('#up_contactRenewalTypeId').val();
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not updated ",
						"updateContactRenewalReminderId", "error");

			} else {

				showNotificationError("updated Successfully",
						"updateContactRenewalReminderId", "success");
				getAllContactRenewalRemindersRef();() ;
		...
		error : function() {
			...
			console.log('In Error of  updateContactRenewalTypeRegistration');
		}
	});
}
...
function contactRenewalReminderDeletion() {
	...
	$
			.ajax({
				...
				success : function(data) {
					...
					if (200 !== responseCode) {
						showNotificationError(" not delete ", "deleteContactReminderId",
								"error");

					} else {
						showNotificationError(
								"Delete contact renewal Reminder  Successfully",
								"deleteContactReminderId", "success");
