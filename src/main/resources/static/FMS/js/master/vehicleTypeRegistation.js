 ...
 * 2020-04-20
 */
function addVehicleTypeRegistration() {
	var desc;
	var vehicleType = $('#vehicleTypeId').val();
	var defaultId  = $('#defaultId').is(":checked");
	 
	 if (defaultId === true || defaultId === 'true') {
		 desc = "Default";
		}
	 else{
		 desc ="NA"; 
	...
	var strUrl = Service.addVehicleTypeRegistration;
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not Inserted ",
						"vehicleTypeRegistrationId", "error");

			} else {

				showNotificationError("Inserted Successfully",
						"vehicleTypeRegistrationId", "success");
				getAllVehicleTypeRef() ;
...
function getAllVehicleTypeRef() {

	$("#vehicleTypeDataTableId").empty();
	var strUrl = Service.getAllVehicleTypeRefDetails;
...
function allVehicleTypeData_DOM_NO_DATA() {
	...
	$("#vehicleTypeDataTableId").append(objDivTag);
}
function allVehicleTypeRefData_DOM(strData) {
	...
	try {
		...
		console.log("vehicleTypeDataTableId" + err);
	}
}
var vms_statusid;
function get_RowData(vms_statusId, vehicleTypeName, vms_desc) {
	//$('#update').modal('show');
	vms_statusid = vms_statusId;
	$('#up_vehicleTypeId').val(vehicleTypeName);

}

function  updateVehicleTypeRegistration(){
	var desc;
	var vehicleType = $('#up_vehicleTypeId').val();
	var defaultId  = $('#up_defaultId').is(":checked");
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not Inserted ",
						"upvehicleTypeRegistrationId", "error");

			} else {

				showNotificationError("Inserted Successfully",
						"upvehicleTypeRegistrationId", "success");
				getAllVehicleTypeRef() ;
	...
	$
			.ajax({
				...
				success : function(data) {
					...
					if (200 !== responseCode) {
						showNotificationError(" not delete ", "vehicleTypeId",
								"error");

					} else {
						showNotificationError(
								"Delete vehicle status  Successfully",
								"vehicleTypeId", "success");
