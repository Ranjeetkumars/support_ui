 ...
 * 2020-04-20
 */
function addExpenseTypesRegistration() {
	...
	var strUrl = Service.addExpenseTypesRegistration;
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not Inserted ",
						"addExpenseTypesId", "error");

			} else {

				showNotificationError("Inserted Successfully",
						"addExpenseTypesId", "success");
				getAllExpenseTypesRefDetails();
		...
		error : function() {
			...
			console.log('In Error of  addExpenseTypesRegistration');
		}
	});
}

function getAllExpenseTypesRefDetails() {


	$("#expenseTypeDataTableId").empty();
	var strUrl = Service.getAllExpenseTypeRefDetails;
			...
			.ajax({
				...
				success : function(data) {
					...
					if (200 !== responseCode || data.status === "NO_DATA_FOUND") {
						allExpenseTypeData_DOM_NO_DATA();
						...
						if (jsonArray.length > 0) {
							allExpenseTypeRefData_DOM(jsonArray);
							loadDataTable();
						}
					}
				},
...
function allExpenseTypeData_DOM_NO_DATA() {
	...
	$("#expenseTypeDataTableId").append(objDivTag);
}
function allExpenseTypeRefData_DOM(strData) {
	...
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			$(tablcol6).attr('onclick', 'deleteSingleExpenseType()');
		...
		console.log("expenseTypeDataTableId" + err);
	}
}
var expenseTypeId;
function get_RowData(vms_expenseTypeId, expenseType, vms_desc) {
	//$('#update').modal('show');
	expenseTypeId = vms_expenseTypeId;
	$('#up_expenseTypeId').val(expenseType);

}

function  updateExpenseTypeRegistration(){

var expenseType = $('#up_expenseTypeId').val();
	...
	var strUrl = Service.addExpenseTypesRegistration;
	console.log("updateExpenseTypeRegistration  details Url is:"
			+ strUrl);
	console.log("Input is:::::::" + JSON.stringify(objJson));
	$.ajax({
		type : "POST",
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not Inserted ",
						"updateExpenseTypeId", "error");

			} else {

				showNotificationError("update Successfully",
						"updateExpenseTypeId", "success");
				getAllExpenseTypesRefDetails();
		...
		error : function() {
			...
			console.log('In Error of  updateExpenseTypeRegistration');
		}
	});
}
 ...
 * 30-11-2019
 */
function deleteSingleExpenseType() {
...
function expenseTypeDeletion() {
	var fuelType = $('#reg_no').val();
	console.log("delete vehicle Status List====" + fuelType)
	var objJson = {
		"vms_statusid" : fuelType
	}
	var strUrl = Service.deleteExpenseTypeRefDetails;
	console.log("expenseTypeDeletion Url is:" + strUrl);
	console.log("Input is:::::::" + JSON.stringify(objJson));
	$
			.ajax({
				type : "POST",
				...
				success : function(data) {
					...
					if (200 !== responseCode) {
						showNotificationError(" not delete ", "expenseTypeDeletteId",
								"error");

					} else {
						showNotificationError(
								"Delete Expense Type Successfully",
								"expenseTypeDeletteId", "success");
				...
				error : function() {
					...
					console.log('In Error of  expenseTypeDeletion ');
				}
			})
}
