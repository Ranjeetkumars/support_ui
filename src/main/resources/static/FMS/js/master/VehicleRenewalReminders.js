...
$(document).ready(function() {
	try {
		getAllVehicleRenewalRemindersTypeRef();
 ...
 * 2020-04-20
 */
function addVehicleRenewalRemindersRegistration() {

	var vehicleRenewalType = $('#vehicleRenewalTypeId').val();

	

	var objJson = { "condition":1,
			       "renewal_typeid":1, 
			      "renewal_type":vehicleRenewalType,
	...
	var strUrl = Service.addVehicleRenewalRemindersRegistration;
	console.log("addVehicleRenewalRemindersRegistration  details Url is:"
			+ strUrl);
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not Inserted ",
						"vehicleRenewalReminderId", "error");

			} else {

				showNotificationError("Inserted Successfully",
						"vehicleRenewalReminderId", "success");
				getAllVehicleRenewalRemindersTypeRef() ;
		...
		error : function() {
			...
			console.log('In Error of  addVehicleRenewalRemindersRegistration');
		}
	});
}

function getAllVehicleRenewalRemindersTypeRef() {

	$("#vehicleRenewalRemindersDataTableId").empty();
	var strUrl = Service.getAllVehicleRenewalTypeDetails;
			...
			.ajax({
				...
				success : function(data) {
					...
					if (200 !== responseCode || data.status === "NO_DATA_FOUND") {
						allvehicleRenewalRemindersData_DOM_NO_DATA();
						...
						var jsonArray = data.objGetAllVehicleRenewalTypeDetailsControllerDTO;
						if (jsonArray.length > 0) {
							allvehicleRenewalRemindersRefData_DOM(jsonArray);
							loadDataTable();
						}
					}
				},
...
function allvehicleRenewalRemindersData_DOM_NO_DATA() {
	...
	$(objTHead3).html('VehicleRenewal Reminder Type');
	...
	$("#vehicleRenewalRemindersDataTableId").append(objDivTag);
}
function allvehicleRenewalRemindersRefData_DOM(strData) {
	...
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			$(tablcol1)
					.html(
							'<label class="check "><input type="checkbox" id="myCheck12"  class="case"  value='
									+ strData[i].renewal_typeid
									+ ' name="case"  )" ><span class="checkmark"> </label>');
			$(tbleRow).append(tablcol1);
			$('#selectall').val(strData[i].renewal_typeid);
			...
			$(tablcol3).addClass('text-center');
				...
				$(tablcol3).html(strData[i].renewal_type);
			...
			$(tablcol5).attr(
					'onclick',
					'get_RowData("' + strData[i].renewal_typeid + '","'
							+ strData[i].renewal_type + '")');

			var tablcol6 = document.createElement("td");
			$(tablcol6).addClass('text-center');
			$(tablcol6).append(
					'<a href="#"><i class="fa fa-trash"></i><i></a> ');
			$(tablcol6).attr('onclick', 'deleteSingleVehicleType()');
		...
		console.log("vehicleRenewalRemindersDataTableId" + err);
	}
}
var renewal_typeId;
function get_RowData(renewal_typeid, renewal_type) {
	//$('#update').modal('show');
	renewal_typeId = renewal_typeid;
	$('#up_vehilceRenewalTypeId').val(renewal_type);

}

function  updateVehicleRenewalRemindersRegistration(){
	var renewal_type=$('#up_vehilceRenewalTypeId').val();
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("not Inserted ",
						"updateVehicleRenewalReminderId", "error");

			} else {

				showNotificationError("Inserted Successfully",
						"updateVehicleRenewalReminderId", "success");
				getAllVehicleRenewalRemindersTypeRef() ;
...
function vehicleRenewalReminderTypeDeletion() {
	var vehicleType = $('#reg_no').val();
	console.log("delete vehicle Status List====" + vehicleType)
	var objJson = {
		"vms_statusid" : vehicleType
	}
	var strUrl = Service.deleteVehicleRenewalTypeDetails;
	console.log("vehicleRenewalReminderTypeDeletion Url is:" + strUrl);
	console.log("Input is:::::::" + JSON.stringify(objJson));
	$
			.ajax({
				type : "POST",
				...
				success : function(data) {
					...
					if (200 !== responseCode) {
						showNotificationError(" not delete ", "vehicleRenewalReminderTypeDeleteId",
								"error");

					} else {
						showNotificationError(
								"Delete vehicle Renewal Reminder  Successfully",
								"vehicleRenewalReminderTypeDeleteId", "success");
				...
				error : function() {
					...
					console.log('In Error of  vehicleRenewalReminderTypeDeletion ');
				}
			})
}
