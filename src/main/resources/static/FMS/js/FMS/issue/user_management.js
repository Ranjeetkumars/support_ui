...
function updateContact() {
	...
	var uemployeetype = $('#employeetype').val();
...
/**
 * Fun:delete Registation user Details
 * @param serialid
 * date:17/12/2019
 * @returns
 */
...
function deleteContactDetails(serialid) {
...
// for multiple select this function calling.
function MultipleDeleteContact(serialid) {
	console.log("SerialId---->" + serialid);
	var objUpd = {
		"serialno" : serialid
	}
	var strUrl2 = Service.multipledeleteContactList;
	console.log('==== strUrl' + strUrl2);
	$.ajax({
		type : "POST",
		url : strUrl2,
...
/*		var objTHead6 = document.createElement("th");
  ...
  /*      var objTHead7 = document.createElement("th");
		...
		for (var i = 0; i < strData.length; i++) {
            ...
            $(tablcol7).html('<u><a href="#" data-toggle="modal" data-target="#">View</a></u>');
            $(tbleRow).append(tablcol7);
            $(tablcol7).attr('onclick', 'getUserDetailsView("' + strData[i].userId + '")');
			*/
			...
			$(tablcol9).attr('onclick',
					'deleteContactDetails("' + strData[i].serialno + '")');
...
function searchContactUsers() {
	...
	if(nameid==null&&emailId==null&&phoneNo==null){
	 ...
	 var objJson = {
	    ...
	    var strUrl = 'http://192.168.1.191:8085/FleetManagement/contactUsersController/searchContactDetails';
	    console.log("" + strUrl);
	    $.ajax({
	        type: "POST",
	        url: strUrl,
...
function deleteMultipleContactchkboxReading() {
	...
	if (selected.length > 0) {
		...
		MultipleDeleteContact(selected);
...
function onclickCheckbox() {
	...
	deleteMultipleVendorContactDetails1111111111();
}
...
//this function reading selected checkbox..
function deleteMultipleVendorContactDetails1111111111() {
	var chkArray = [];
	$("#tbodyData input:checked").each(function() {
		chkArray.push($(this).val());
	});
