...
//	var leaveDate1 = moment(leaveDate).format("YYYY-MM-DD");
    ...
    var leaveDate1 = $("#leavedate").val();
    var d = new Date(leaveDate1.split("-").reverse().join("-"));
    var dd = d.getDate();
    var mm = d.getMonth() + 1;
		...
		/*		
				...
				success : function(data) {
					...
					if (200 !== responsecode) {
						...
						showNotificationError("Successfully Inserted ","save_user_details", "success");
//						window
//								.setTimeout(
//										function() {
//											window.location
...
/*	if (uemployeeno === "0" || uemployeeno === "" || uemployeeno === null) {
	...
	alert("updateContact22222222222222222222222222222");
	...
	$.ajax({
		 ...
		 */
		success : function(data) {
			alert("updateContact============="+data.responseCode);
...
function getContactUsers() {
	...
	var strUrl = Service.getContactUsers;
	console.log("" + strUrl);
	$.ajax({
		type : 'GET',
		url : strUrl,
...
function searchContactUsers(empType) {
	...
	if(nameid==null&&emailId==null&&phoneNo==null){
	 ...
	 var strUrl = Service.searchContactUsers;
	    console.log("" + strUrl);
	    $.ajax({
	        type: "POST",
	        url: strUrl,
...
function getUserRoles() {
	$('#role_id').empty();
	 var strUrl = Service.getUserRoles;
	// $('#employeeTypeUpdate').empty();
	$.ajax({
		type : 'GET',
		url : strUrl,
