...
/*
 * @DESC : getDistrictBasedonStateId @AuthorName : priyadarshini @DATE :
 * 2020-01-10
 */
	...
	try {
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						if (200 !== responseCode) {
							...
							$.each(jsonArray, function(i, resData) {
								var District = "<option value="
										+ resData.districtId + ","+ resData.districtName+ ">"
					...
					error : function(err) {
						console.error("Error in getDistrictBasedonStateId"
								+ JSON.stringify(err));
					}
				});
	} catch (err) {
 ...
 * @AuthorName : priyadarshini @DATE : 2020-01-10
 */
$('#reg_districtId').on('change', function() {
	var district = $('#reg_districtId').val();
	var distrctid=district.split(",");
	var districtId = distrctid[0];
	$('#reg_mandalId').empty();
	getMandalBasedonDistrictId(districtId, 'reg_mandalId');
});
/*
 * @DESC : getMandalBasedonDistrictId @AuthorName : priyadarshini @DATE :
 * 2020-01-10
 */
				...
				.ajax({
					...
					success : function(data) {
						...
						if (200 !== responseCode) {
							...
							$.each(jsonArray, function(i, resData) {
								var District = "<option value="
										+ resData.mandalId + ","+ resData.mandalName + ">"
					...
					error : function(err) {
						console.error("Error in getMandalBasedonDistrictId"
								+ JSON.stringify(err));
					}
				});
	} catch (err) {
...
/*
 * /*@DESC : onchange method for loading city based on mandal id @AuthorName :
 * priyadarshini @DATE : 2020-01-10
 */
$('#reg_mandalId').on('change', function() {
	var mandalid = $('#reg_mandalId').val();
	var mandal=mandalid.split(",");
	$('#reg_cityId').empty();
	getCityBasedonMandalId(mandal[0], 'reg_cityId');
});
/*
 * @DESC : getCityBasedonMandalId @AuthorName : priyadarshini @DATE : 2020-01-10
 */
				...
				.ajax({
					...
					success : function(data) {
						...
						if (200 !== responseCode) {
							...
							$(id).append(selectfirst);
							$.each(jsonArray, function(i, resData) {
								var city = "<option value=" + resData.cityId
										+","+ resData.cityName+ ">" + resData.cityName + "</option>";
								$(city).appendTo(id);
							});
						}
					},
					error : function(err) {
						console.error("Error in getCityBasedonMandalId"
								+ JSON.stringify(err));
					}
				});
	} catch (err) {
...
/*
 * @DESC : saveServicePointDetails @AuthorName : priyadarshini @DATE :
 * 2020-01-10
 */
function saveServicePointDetails() {
	...
	var mandalid=mandal.split(",");
	var Latitude = $('#reg_LatitudeId').val();
	var city = $('#reg_cityId').val();
	var cityid=city.split(",");
	var longitide = $('#reg_longitideId').val();
	var addressid=distrctid[1]+","+mandalid[1]+","+cityid[1]
	var re = /^[-+]?[0-9]+\.[0-9]+$/;
	...
	if (district === "0") {
		...
		showNotificationError("Enter servicePoint name", "reg_servicePointId",
				"error");
		return;
	}
		...
		if (servicePoint.match(regx2)) {
			...
			showNotificationError("please enter valid servicePoint name",
					"reg_servicePointId", "error");
			return;
		}
		...
		showNotificationError("please enter valid Latitude", "reg_LatitudeId",
				"error");
		...
		showNotificationError("please enter valid Longitude",
				"reg_longitideId", "error");
		return;
	}
	...
	$.ajax({
		...
		success : function(data) {
			var responseCode = data.responseCode;
			if (200 !== responseCode) {
				showNotificationError("not Inserted ", "servicePointDetailsId",
						"error");
			} else {

				showNotificationError("Inserted Successfully",
						"servicePointDetailsId", "success");
...
/*
 * For getServicePointRegistationDetails. priyadarshini 30-11-2019 inputs :no
 */
...
/*
 * For data table loading. priyadarshini 30-11-2019
 * 
 */
function servicePointRegistation_DOM(strData) {
	try {
		...
		for (var i = 0; i < strData.length; i++) {
			...
			$(tablcol8)
					.append(
							'<a href="#"><i  class="fa fa-edit" data-toggle="modal" data-target="#updateId"></i><i></a> ');
			$(tablcol8).attr(
					'onclick',
					'get_RowData("' + strData[i].servicepointName + '","'
							+ strData[i].servicepointId + '","'
							+ strData[i].districtId + '","'
							+ strData[i].districtName + '","'
							+ strData[i].cityId + '","' + strData[i].cityName
							+ '","' + strData[i].mandalId + '","'
			...
			$(tablcol9).attr(
					'onclick',
					'deleteServicePointRegistationData("'
							+ strData[i].servicepointId + '")');
...
var serialId;
function get_RowData(servicepointName, servicepointId, districtId,
		districtName, cityId, cityName, mandalId, mandalName, latitude,
		longitude) {
	getDistrictBasedonStateId('up_districtId');
	$("#up_districtId option:contains(" + districtName + ")").attr('selected',
			'selected').trigger("chosen:updated");
	getMandalBasedonDistrictId(districtId, 'up_mandalId');
	$("#up_mandalId option:contains(" + mandalName + ")").attr('selected',
			'selected').trigger("chosen:updated");
	getCityBasedonMandalId(mandalId, 'up_cityId');
	$("#up_cityId option:contains(" + cityName + ")").attr('selected',
			'selected').trigger("chosen:updated");
...
/*
 * For function deleteServicePointRegistationData priyadarshini 10-01-2020
 */
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("delete Successfully", "deleteid",
						"success");
...
/*
 * For function resetServicePointRegistationDetails priyadarshini 10-01-2020
 */
function resetServicePointRegistationDetails() {
	 ...
	 * For function updateServicePointRegistation priyadarshini 10-01-2020
	 */
...
function updateServicePointRegistation() {
	...
	if (up_district === "0") {
		...
		showNotificationError("Enter servicePoint", "up_servicePointNameId",
				"error");
		return;
		...
		showNotificationError("please enter valid Latitude", "up_latitudeId",
				"error");
		...
		showNotificationError("please enter valid Longitude", "up_longitudeId",
				"error");
		return;
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (200 !== responseCode) {
				...
				showNotificationError("updated Successfully", "update_id",
						"success");
				window.setTimeout(function() {
					location.reload();
				}, 2000);
...
/*
 * For function deleteServicePointRegistationData1 priyadarshini 10-01-2020
 */
function deleteServicePointRegistationData1() {
	...
	$.ajax({
		...
		success : function(data) {
			var responseCode = data.responseCode;
			if (200 !== responseCode) {
				showNotificationError("not deleted ", "servicePointDeleteId",
						"error");
			} else {

				showNotificationError("Deleted Successfully",
						"servicePointDeleteId", "success");
				window.setTimeout(function() {
					location.reload();
				}, 2000);
			}
