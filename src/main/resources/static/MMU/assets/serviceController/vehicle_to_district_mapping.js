...
/**
 * @desc:Assign Vehicle to District
 * @returns:
 */
 function assignDistrictToVehicle(){
    	...
    	if(district_id=="0"||district_id=='0'){
    	 ...
    	 if(unassignvehicleId==null||unassignvehicleId=="0"||unassignvehicleId==""){
 ...
 function unAssignVehicle(){
	 var  vehicle_id=$('#assign_vehicleId').val();
		 
	 if(vehicle_id=="0"||vehicle_id==""||vehicle_id==null){
		  showNotificationError("Please Select Vehicle", "assign_vehicleId", "error");
	 return;
	  }
	     var vehicleid=vehicle_id.toString();
		 var vehid=vehicleid.split(",");
		 for(var i=0;i<vehid.length;i++){
			 deAssignVehicle(vehid[i]);
		 }
 ...
 //deAssign Vehicle
 function deAssignVehicle(vehicleid){
	 var objson={
			  "vehicleId":vehicleid,	
    };

console.log("ObJson---->" + JSON.stringify(objson));
var strUrl = Service.deAssignVehicle;
...
//district dropdown function
 ...
 /**
  * @desc:UnAssign Vehicle to District
  * @returns:
  */
...
/**
 * @desc:Getting UnAssign Vehicle of District
 * @returns:
 */
function getUnAssignVehicleList() {
	...
	/**
	 * @desc:Getting Assign vehicle of District
	 * @returns:
	 */
...
/**
 * @desc Getting Service point Registation Details
 * 30-11-2019
 * inputs :no
 */
